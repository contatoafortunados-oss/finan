(() => {
  ...
  const render = () => {
    ...
    const mount = () => {
      ...
      const visibleRows = importableRows(data.rows), total = visibleRows.reduce((sum, row) => sum + (Number(row.amount) || 0), 0), card = document.createElement('section');
      card.id = 'spreadsheet-preview'; card.className = 'card spreadsheet-preview import-review-redesign';
      ...
      const oldTitle = card.querySelector('.section-title'), oldActions = card.querySelector('.review-actions');
      const hero = document.createElement('div'); hero.className = 'import-review-hero';
      hero.innerHTML = `<div class="import-review-file"><span class="import-file-icon">▤</span><div><strong>Revisão de importação</strong><small>Arquivo: ${esc(data.fileName)} · Banco: ${esc(data.bank || 'Revisão necessária')} · Cartão: ${esc(data.card || 'Não identificado')} · Mês: ${esc(data.referenceMonth || 'Revisão necessária')}</small></div></div><div class="import-review-kpi"><small>Total da fatura</small><b>${money(total)}</b></div><div class="import-review-kpi"><small>Importado</small><b>${money(total)}</b></div><div class="import-review-kpi"><small>Diferença</small><b class="up">R$ 0,00</b></div><div class="import-review-kpi"><small>Status</small><b class="up">✓ ${data.remoteBatchId ? 'Enviada' : 'Em revisão'}</b></div><div class="import-review-count"><small>Lançamentos</small><b>${data.rows.length}</b></div>`;
      oldTitle?.before(hero); if (oldActions) hero.append(oldActions);
      const notice = document.createElement('div'); notice.className = 'import-review-notice'; notice.innerHTML = '<strong>Revise os lançamentos importados antes de confirmar.</strong><span>Após a confirmação, o lote será enviado ao Supabase. Corrija somente as linhas que estiverem incorretas.</span>'; hero.after(notice);
