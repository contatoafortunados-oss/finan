(() => {
  ...
  const renderReal = async () => {
    if (loading || window.financeState?.view !== 'dashboard' || !window.clarezaDb) return;
    ...
    try {
      ...
      content.dataset.realDashboard = 'false'; content.innerHTML = `<div class="card"><h2>Não foi possível carregar os dados reais</h2><p class="muted">${escapeHtml(error.message)}</p></div>`;
